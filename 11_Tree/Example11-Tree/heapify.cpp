...
void Heapify(vector<ElemType> &x, int low, int high) {
  ...
  while (large_index <= high) {
    if (large_index < high && x[large_index] < x[large_index+1]) {  //檢查是否存在右節點且右節點大於左節點
      large_index = large_index + 1;    //將目標轉為右節點, 方便比較右節點(為子節點中最大)
    }
    if (temp > x[large_index]) {
      ...
      x[low] = x[large_index];
      low = large_index;
      large_index = 2 * low + 1;    //繼續比較, 若爆表則將temp給x[low]
    }
  }
  x[low] = temp;
}
